...
function CartScreen({ navigation }) {
    ...
    const fetchCartItems = async () => {
      try {
        ...
        if (user) {
          ...
          const response = await axios.get(`http://172.20.192.1:8080/api/carts/${userId}`);
  ...
  const updateCartItemQuantity = async (userId, itemKey, quantity) => {
    try {
      await axios.post(`http://172.20.192.1:8080/api/carts/${userId}/${itemKey}`, null, { params: { quantity } });
  ...
  const handleRemoveItem = async (item) => {
    try {
      const response = await axios.delete(`http://172.20.192.1:8080/api/carts/${item.itemKey}`);
