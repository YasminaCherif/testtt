...
export const addFavorite = async (platId) => {
  ...
  if (user) {
    try {
      if (platId) {
        const response = await axios.post('http://172.20.192.1:8080/favorites/add', null, {
          params: {
            userId: user.uid,
            platId: platId,
          },
...
export const removeFavorite = async (platId) => {
  ...
  if (user) {
    try {
      const response = await axios.delete('http://172.20.192.1:8080/favorites/remove', {
        params: {
          userId: user.uid,
          platId: platId,
        },
...
export const fetchFavorites = async () => {
  ...
  if (!user) return [];
  try {
    const response = await axios.get('http://172.20.192.1:8080/favorites/fetch', {
      params: {
        userId: user.uid,
      },
    });
